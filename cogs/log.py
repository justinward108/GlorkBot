...
class logging(interactions.Extension):
	...
	def __init__(self,client: interactions.Client):
		logging.client: interactions.Client = client
	...
	async def ext_command(self, ctx: interactions.CommandContext, channel: str):
		logging.channel_id[str(ctx.guild_id)]=str(channel.id)
		with open ('channels.json','w') as outfile:
			json.dump(logging.channel_id, outfile)
	...
	async def ext_command1(self, ctx: interactions.CommandContext):
		if ctx.guild_id not in logging.channel_id.keys():
			await ctx.send("Logging was not enabled")
			return
		removed_value = logging.channel_id.pop(ctx.guild_id)
		with open('channels.json','w') as file:
			json.dump(logging.channel_id, file)	
	...
	async def on_message_delete(self, message: interactions.Message):
		...
		if str(message.guild_id) not in logging.channel_id.keys():
			return
		channel = await interactions.get(config.bot, interactions.Channel, object_id=logging.channel_id[str(message.guild_id)])
	...
	async def on_message_update(self, before, after):
		...
		if before.author.bot:
			return
		if str(before.guild_id) not in logging.channel_id.keys():
			return
		if before.content != after.content:
			channel = await interactions.get(config.bot, interactions.Channel, object_id=logging.channel_id[str(before.guild_id)])
